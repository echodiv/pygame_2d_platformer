class Collision(object):
    def bullet_and_target(bullet, target):
        if (
            bullet.y - bullet.radius < target.hitbox[1] + target.hitbox[3]
            and bullet.y + bullet.radius > target.hitbox[1]
        ):
            if (
                bullet.x + bullet.radius > target.hitbox[0]
    ...
    def enemy_attack(enemy, hero):
        if (
            hero.hitbox[1] < enemy.hitbox[1] + enemy.hitbox[3]
            and hero.hitbox[1] + hero.hitbox[3] > enemy.hitbox[1]
        ):
            if (
                hero.hitbox[0] + hero.hitbox[2] > enemy.hitbox[0]
    ...
    def hero_on_block(block, hero):
        if (
            hero.hitbox[0] > block.hitbox[0]
            and hero.hitbox[0] < block.hitbox[0] + block.hitbox[2]
        ):
